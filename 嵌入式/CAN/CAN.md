...
**RTR位**(Remote Request)：远程数据请求位
**SRR位**( Substitute Remote Request)：替代远程数据请求位
- 作用1：充当占位符，兼容标准帧
- 作用2：提高标准帧的优先级，SRR为隐性电平(1),在仲裁时如果发送相同的id时就会优先读取标准帧

#### 控制段

标准帧是由IDE（1位）+r0（1位）+DLC（4）组成
拓展帧是由r1（1位）+r0（1位）+DLC（4位）组成
